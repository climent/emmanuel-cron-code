...
const float SNOW_SPARKLE_BEAT_LEVEL_THRESHOLD_RATIO = 0.8;
...
const float GRADUAL_FILL_BEAT_LEVEL_THRESHOLD_RATIO = 0.75;
...
const uint16_t BIKE_SHAPES_MIN_RAND_FRAMES_BETWEEN_CHANGES = 12;
const uint16_t BIKE_SHAPES_MAX_RAND_FRAMES_BETWEEN_CHANGES = 60;
const uint16_t BIKE_SHAPES_MIN_FRAMES_BETWEEN_CHANGES = 6;
const float BIKE_SHAPES_BEAT_LEVEL_THRESHOLD_RATIO = 0.75;
bool bikeShapesMode() {
  static uint16_t lastShapeChangeFrame[3] = {0, 0, 0};
  static uint8_t framesBetweenChanges[3] = {0, 0, 0};

  bool newFrontColor = false;
  bool newMiddleColor = false;
  bool newRearColor = false;

  bool change = false;
  uint8_t additionalFramesBetweenChanges = 0;
  if (param2 < CUSTOM_ANALOG_OPTION_1_THRESHOLD) {
    additionalFramesBetweenChanges = 60;
    if (beatLevel >= (decayingBeatLevelPeak *
                      BIKE_SHAPES_BEAT_LEVEL_THRESHOLD_RATIO)) {
      change = true;
    }
  }

  // Front.
  if ((change && abs(videoFrameCounter - lastShapeChangeFrame[0])
       > BIKE_SHAPES_MIN_FRAMES_BETWEEN_CHANGES)
      || (abs(videoFrameCounter - lastShapeChangeFrame[0])
          >= framesBetweenChanges[0])) {
    lastShapeChangeFrame[0] = videoFrameCounter;
    newFrontColor = true;

    framesBetweenChanges[0] =
      additionalFramesBetweenChanges + random(
        BIKE_SHAPES_MIN_RAND_FRAMES_BETWEEN_CHANGES,
        BIKE_SHAPES_MAX_RAND_FRAMES_BETWEEN_CHANGES + 1);
  }

  // Middle.
  if ((change && abs(videoFrameCounter - lastShapeChangeFrame[1])
       > BIKE_SHAPES_MIN_FRAMES_BETWEEN_CHANGES)
      || (abs(videoFrameCounter - lastShapeChangeFrame[1])
          >= framesBetweenChanges[1])) {
    lastShapeChangeFrame[1] = videoFrameCounter;
    newMiddleColor = true;
  ...
  // Rear.
  if ((change && abs(videoFrameCounter - lastShapeChangeFrame[2])
       > BIKE_SHAPES_MIN_FRAMES_BETWEEN_CHANGES)
      || (abs(videoFrameCounter - lastShapeChangeFrame[2])
          >= framesBetweenChanges[2])) {
    lastShapeChangeFrame[2] = videoFrameCounter;
    newRearColor = true;
  ...
  return bikeShapesEffect(newFrontColor, newMiddleColor, newRearColor);
}
